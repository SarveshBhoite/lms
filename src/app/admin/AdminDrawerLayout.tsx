...
export default function AdminDrawerLayout({
  ...
  const totalBadgesCount = unreadNotificationsCount;
  ...
  const navigation = [
    { name: "Dashboard", href: "/admin/dashboard", icon: LayoutDashboard },
    {
      ...
      badgeTooltip: "Registered students",
    },
    { name: "Trainers & Faculty", href: "/admin/trainers", icon: GraduationCap },
    { name: "Course Catalog", href: "/admin/courses", icon: BookOpen },
    { name: "Content Library", href: "/admin/content", icon: FileBox },
    {
      name: "Batches & Cohorts",
      href: "/admin/batches",
      icon: Layers,
      badge: activeBatchesCount,
      badgeTooltip: "Active batches",
    },
    { name: "Attendance Control", href: "/admin/attendance", icon: CheckSquare },
    { name: "Enrollments", href: "/admin/enrollments", icon: Users },
    { name: "Certificates Registry", href: "/admin/certificates", icon: Award },
    {
      name: "Notifications",
  ...
  return (
    <div className="min-h-screen h-screen portal-bg-mesh flex flex-col md:flex-row selection:bg-purple-500 selection:text-white relative overflow-hidden">
      ...
      {/* MOBILE TOP BAR (visible on screens < md, matches Trainer and Student exactly) */}
      <header className="md:hidden h-16 border-b border-slate-200/80 bg-white/95 backdrop-blur-md px-4 flex items-center justify-between sticky top-0 z-40 shrink-0 shadow-xs">
        ...
        {/* Mobile Hamburger button with unread alerts badge pill */}
        <button
                  ...
                  >
                    ...
                    {item.badge !== undefined && item.badge > 0 ? (
                      <span
                        ...
                        className="px-2 py-0.5 rounded-full bg-purple-100 text-[#7C248C] font-mono text-[10px] font-bold shadow-xs border border-purple-200"
                      >
