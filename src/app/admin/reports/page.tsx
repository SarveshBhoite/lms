...
export default async function AdminReportsPage() {
  ...
  // Fetch institutional real-time metrics across all courses and batches
  ...
  // Overall institute attendance turnout rate
        ...
        <div className="flex items-center gap-4 relative z-10 min-w-0">
          ...
          <div className="space-y-1 min-w-0">
            ...
            <p className="text-slate-500 text-xs font-mono">
              Generate auditable registries, student progress metrics, and evaluation reports across all academy programs in Excel (.xlsx) & CSV.
            </p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-4">
        <div className="p-4 rounded-2xl bg-white border border-slate-200/80 shadow-xs flex flex-col justify-between">
          <span className="text-[11px] font-bold text-slate-500 uppercase tracking-wider font-mono flex items-center gap-1.5">
            <BookOpen className="w-3.5 h-3.5 text-[#1E2B88]" /> All Courses
          </span>
        ...
        <div className="p-4 rounded-2xl bg-white border border-slate-200/80 shadow-xs flex flex-col justify-between">
          <span className="text-[11px] font-bold text-slate-500 uppercase tracking-wider font-mono flex items-center gap-1.5">
            <Award className="w-3.5 h-3.5 text-emerald-600" /> Institute Turnout %
          </span>
          <div className="mt-2 flex items-baseline gap-2">
            <span className="text-2xl font-black text-emerald-700">
              {overallTurnoutPct > 0 ? `${overallTurnoutPct}%` : "—"}
            </span>
            <span className="text-[11px] font-mono text-slate-400">Attendance</span>
          </div>
        </div>
